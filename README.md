...
- ⚡ **One key fails → next key automatically**: if an account hits a 429 (`Daily free limit reached`), an empty response or a 5xx, the same request is instantly retried on the next account (with short backoff). Transient 502/503/529 overloads get a quick retry with only a 30s cooldown (never hours). All-accounts-cooling returns the upstream response as-is instead of spinning.
- 🧱 **Built for huge codebases**: large prompts (tested: 1.1 MB in one request), parallel bursts (tested: 5 concurrent) and long streaming generations (tested: 220 KB SSE to `[DONE]`) all pass through safely. If a prompt exceeds the selected model's context window, the request is automatically retried once on `deepseek/deepseek-v4-flash` (1M context) and the response carries an `X-Cline-Fallback` header saying so. Tip: prefer `stream: true` for long generations — first bytes arrive immediately instead of waiting on full aggregation.
